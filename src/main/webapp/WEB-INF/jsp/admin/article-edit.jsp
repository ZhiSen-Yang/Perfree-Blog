...
<body>
	...
	编辑文章 <a class="btn btn-success radius r"
				...
				<span class="select-box"> <select class="select" size="1"
					name="parentId" onchange="parent()" id="pid">
						...
						<c:forEach items="${parent}" var="parentBoard">
							<c:choose>
								<c:when test="${parentBoard.id == child[0].parentId}">
									<option value="${parentBoard.id }" selected="selected">${parentBoard.name }</option>
								</c:when>
								<c:otherwise>
			...
			<div class="formControls col-xs-8 col-sm-2">
				<span class="select-box"> <select class="select" size="1"
					name="boardId" id="child">
						<!-- 动态追加option -->
						<option value="0">---请选择----</option>
							<c:forEach items="${child}" var="childBoard">
								<c:choose>
									<c:when test="${childBoard.id == article.boardId}">
										<option value="${childBoard.id }" selected="selected">${childBoard.name }</option>
									</c:when>
									<c:otherwise>
										<option value="${childBoard.id }">${childBoard.name }</option>
									</c:otherwise>
								</c:choose>
							</c:forEach>
				</select>
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-6">
				<div id="fengmiannBox" align="center">
					<img alt='' src='${article.imageUrl }' width='150px' height='100px' id='fengmianImg'>
				</div>
				<span class="btn-upload form-group"> <input type="hidden" name="imageUrl" id="imageUrl" value="${article.imageUrl }"><a href="javascript:void();"
		...
		<div class="row cl">
			<div class="formControls col-xs-12 col-sm-12">
				<div id="test-editormd">
					<textarea style="display: none;" name="context" id="context">
${article.context }</textarea>
	...
	<script type="text/javascript">
		...
		<c:forEach items="${articleTags}" var="articleTag">  
			tagIds.push("${articleTag.tagId}"); //js中可以使用此标签，将EL表达式中的值push到数组中  
		</c:forEach>
	$(function(){
		$.each(tagIds,function(index,item){
			$("#uc_04").children("option").each(function(){
			               var temp_value = $(this).val();
			              if(temp_value == item){
			               var index = $(this).index();
		                    $(this).attr("selected","selected");
			               $(this).parent("select").siblings("ul").children("li:eq("+index+")").attr("class","selected");
			              }
			         });
		})
	});
		...
		$("#articleForm").on("click", function() {
			...
			layer.confirm('确定要更新此文章吗?',function(index) {
				$.ajax({
					type: 'POST',
			...
			$('#form-article-send').ajaxSubmit( //ajax方式提交表单
			{
				...
				success : function(data) {
					if (data.status == 200) {
						layer.confirm('文章已更新', {btn: ['确定'], title: "提示"},function(index) {
